...
//      " appsink name=sink caps=\"" CAPS "\"", argv[1]);
  descr = g_strdup_printf ("\
      videomixer name=mix ! videoconvert ! appsink sync=false name=sink \
...
//  descr = g_strdup_printf ("\
//      v4l2src device=/dev/video2 ! video/x-raw, format=RGB, framerate=30/1, width=320, height=240 ! \
//      videoconvert ! appsink sync=false name=sink");
...
//    Mat image(width, height, CV_8UC3, map.data);
//    Mat gray_image;
...
//    int i;
//    FILE *fp = fopen("run.m", "w");
//    fprintf (fp, "im = uint8([");
//    for (i = 0; i < width * height * 3; ++i) {
//      fprintf (fp, "%d ", map.data[i]);
//    }
//    fprintf (fp, "]);\n");
//    fprintf (fp, "im = reshape(im, %d, %d, 3);\n", width, height);
//    fprintf (fp, "imshow(im)\n");
//    fclose(fp);
  ...
  Mat image(100, 100, CV_8UC3, Scalar(0, 127, 0));
//  namedWindow("Display", WINDOW_AUTOSIZE);
  imshow("Display", image);
//  waitKey(0);
